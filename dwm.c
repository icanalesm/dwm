...
enum { SchemeNorm, SchemeSel, SchemeTagNorm, SchemeTagSel, SchemeLayout, SchemeStatus}; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (occ & 1 << i)
			drw_rect(drw, x + inds, 0, w - (2 * inds), indh, 1, urg & 1 << i);
		x += w;
	}
	/* Draw window title */
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel && showtitle) {
			...
			if (m->sel->isfloating && showfloating)
				drw_rect(drw, x + i + boxs, boxs, boxw, boxw, m->sel->isfixed, 0);
		} else {
